...
export default {
  ...
  state() {
    return {
      apiUID: '96e0cc51-a62e-42ca-acee-910ea7d2a241',
      apiPwd: '',
    ...
    fetchTransactions({ state, commit }) {
      ...
      return new Promise((resolve, reject) => {
        axios({
          ...
          url: baseURL,
          auth: {
            username: state.apiUID,
            password: state.apiPwd,
          },
          //timeout: 60000,
            ...
            return stuff.data.map(function (item) {
              return [
                item.timeStamp, 
                {
                  ...
                  amount: item.amount as BigNumber,
                  ...
                  subTransactions: item.subTransactions.map(function (subm) {
                      if (item.address == '0x0000000000000000000000000000000000000000') {
                        return {
                          address: null,
                          amount: subm.amount.toString() as BigNumber,
                          symbol: subm.symbol,
                  ...
                  txGas: item.gas.toString() as BigNumber | null,
                  txGasLimit: item.gasLimit.toString() as BigNumber | null,
                  input: (() => {
                    if (item.input == '0x') {
                      return null
                    } else {
                  ...
                  cost: item.gas.toString() as BigNumber,
