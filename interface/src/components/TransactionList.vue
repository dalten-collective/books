    ...
    <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
      <div class="overflow-hidden">
        <table class="min-w-full">
          ...
          <tbody v-for='[kex, details] in paginateTransactions' :key='details.hash' class="bg-white border-b transition duration-300 ease-in-out hover:bg-gray-100">
          ...
          <div class="receive" v-if="chkStep(details.subTransactions, 'incoming')">
            ...
            <div class="step" v-for="step in details.subTransactions" props="{{details}}">
              <div v-if="step.type == 'incoming'">
                {{ step.amount.toSignificantDigits(5) + " " + step.symbol + " from "}}
              </div>
          ...
          <div class="send" v-if="chkStep(details.subTransactions, 'outgoing')">
            ...
            <div class="step" v-for="step in details.subTransactions" props="{{details}}">
              <div v-if="step.type == 'outgoing'">
                {{ step.amount.toSignificantDigits(5) + " " + step.symbol + " to " }}
              </div>
