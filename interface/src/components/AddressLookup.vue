<template>
  <div class='has-tooltip' v-if="showingDetails">
    <span class='tooltip rounded shadow-lg p-1 bg-gray-100 text-red-500'>
      {{addy}}
    </span>
  </div>
...
export default defineComponent ({
    ...
    methods: {
      checkAddress(map, fmap, addy) {
        const myWal = Immutable.Map(map) as Map<[Address, {nickname: string, tags: Array<string>}]> 
        const frenWal = Immutable.Map(fmap) as Map<[Address, WalletDetails]>
        if (Immutable.has(myWal, addy)) {
          return Immutable.get(myWal,addy).nick ? Immutable.get(myWal,addy).nick : this.truncateAddress(addy);
        } else if (Immutable.has(frenWal, addy)) {
          return Immutable.get(frenWal, addy).nick ? Immutable.get(frenWal, addy).nick : this.truncateAddress(addy);
        } else {
