...
export default defineComponent({
  computed: {
    ...mapState('books', ['notes', 'myFriends', 'myWallets']),
    annotations() {
      return Immutable.Map(this.notes)
    },
    people() {
      const friends = (() => {
        return this.myFriends.map((item) => {
          return {
            value: item[0],
            label: (() => {
              if (!item[1].nick) {
                return this.truncateAddress(item[0]);
              } else {
                return item[1].nick;
              }
            })(),
      ...
      const wallets = (() => {
        return this.myWallets.map((item) => {
          return {
      ...
      return wallets.concat(friends)
    },
    formState() {
      return reactive({
        annotated: (() => {
          return Immutable.has(this.annotations, this.hash) as boolean;
        })(),
        basis: (() => {
          if (Immutable.has(this.annotations, this.hash)) {
            return Immutable.get(this.annotations, this.hash).basis as Decimal;
          } else {
        ...
        to: (() => {
          if (Immutable.has(this.annotations, this.hash)) {
            return Immutable.get(this.annotations, this.hash).to as Address | null;
          } else {
